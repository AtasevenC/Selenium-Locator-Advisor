...
const ResultDisplay: React.FC<Props> = ({ markdown }) => {
  ...
  if (!markdown) {
    return (
      <div className="h-full flex items-center justify-center text-slate-400 flex-col p-8 text-center">
        <div className="w-16 h-16 border-2 border-slate-200 rounded-full flex items-center justify-center mb-4">
            <div className="w-8 h-8 bg-slate-200 rounded-full"></div>
        </div>
        <p className="text-lg font-medium">Ready to analyze</p>
        <p className="text-sm mt-2">Enter element details or page HTML on the left to generate locators.</p>
      </div>
    );
  }
  ...
  return (
    <div className="relative bg-white rounded-lg shadow-sm border border-slate-200 overflow-hidden">
       <div className="absolute top-4 right-4 z-10">
        <button
          onClick={handleCopy}
          className="flex items-center gap-1 px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-600 text-xs font-medium rounded transition-colors"
        >
          {copied ? <Check className="w-3 h-3" /> : <Copy className="w-3 h-3" />}
          {copied ? 'Copied' : 'Copy'}
        </button>
      </div>
      <div className="p-8 overflow-auto max-h-[calc(100vh-8rem)] prose prose-slate prose-sm max-w-none">
        <ReactMarkdown 
            ...
            components={{
                ...
                code: ({node, className, children, ...props}) => {
                    ...
                    return !className ? (
                        <code className="bg-slate-100 text-brand-700 px-1 py-0.5 rounded font-mono text-xs" {...props}>
                            {children}
                        </code>
                    ) : (
                        <code className={`${className} block bg-slate-900 text-slate-100 p-4 rounded-lg overflow-x-auto text-xs font-mono`} {...props}>
                            {children}
                        </code>
                    )
                },
                ...
                ul: ({node, ...props}) => <ul className="list-disc list-inside space-y-1 text-slate-600 my-2" {...props} />,
            }}
        >
          {markdown}
        </ReactMarkdown>
